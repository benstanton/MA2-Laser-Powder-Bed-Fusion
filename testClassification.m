...
layers = [
    ...
    fullyConnectedLayer(floor(num_features*.4))         % FC layer
    batchNormalizationLayer         % Batch normalisation
    reluLayer                       % ReLU activation function

    fullyConnectedLayer(floor(num_features*.15))          % FC layer
    batchNormalizationLayer         % Batch normalisation
    reluLayer                       % ReLU activation function
...
[YPred,~] = classify(net,X_val);
accuracy = 100*mean(YPred == Y_val); % accuracy
disp(["with DTNC+hatchLength Validation Accuracy: " num2str(accuracy) "%"]);
